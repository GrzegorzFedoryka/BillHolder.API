...
internal sealed class UploadBillCommandHandler : IRequestHandler<UploadBillCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> Handle(UploadBillCommand request, CancellationToken cancellationToken)
    {
        ...
        var blobClient = containerClient.GetBlobClient(bill.Id.Value.ToString() + "." + request.Format!.FileExtensions.First());
        ...
        bill.UpdateUrl(blobClient.Uri.AbsolutePath);
        await _db.Container.UpsertItemAsync(bill, cancellationToken: cancellationToken);
